...
> * DML의 유형 (SE-IN-DE-UP)
>
> > * **SE**LECT : 테이블에서 튜플을 <u>검색</u>한다. (SELECT ~ FROM ~ WHERE)
> > * **IN**SERT : 테이블에서 새로운 튜플을 <u>삽입</u>한다. (INSERT INTO~ VALUES~)
> > * **DE**LETE : 테이블에서 튜플을 <u>삭제</u>한다. (DELTE~FROM~WHERE~)
> > * **UP**DATE : 테이블에서 튜플의 내용을 <u>갱신(변경)</u>한다. (UPDATE~SET~WHERE~)
> >
> > 
>
> * 삽입문(INSERT INTO~)
...
> > * 모든 레코드를 삭제하더라도 '테이블 구조'는 남아있기 때문에 디스크에서 테이블을 완전히 제거하는 DROP과는 다르다. 
>
> ```plsql
> DELETE 
> FROM 테이블명
...
> > WHERE 이름 = '홍길동';
> > 
> > /* <사원> 테이블에서 "황진이" 의 '부서'를 "기획부"로 변경하고 '기본급'을 5만원 이상 인상시키시오 */
> > UPDATE 사원
